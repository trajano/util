...
%>
<form>
	EJB Home: <input name="ejb" size="100"
		value="<%=ejbName == null ? "" : ejbName%>" /><br />
	<%
		...
		<tbody>
			<%
			    for (int i = 0; i < homeMethods.length; ++i) {
			            ...
			            if (!homeMethods[i].getDeclaringClass().getName()
			                    .startsWith("java")
			                    && !homeMethods[i].getDeclaringClass().getName()
			                            .startsWith("org.omg")) {
			%>
			<tr>
				<td><%=method%></td>
				<td><%=method.getDeclaringClass().getName()%></td>
			</tr>
			<%
			    }
			        }
		...
		<tbody>
			<%
			    for (int i = 0; i < objectMethods.length; ++i) {
			            ...
			            if (!objectMethods[i].getDeclaringClass().getName()
			                    .startsWith("java")
			                    && !objectMethods[i].getDeclaringClass().getName()
			                            .startsWith("org.omg")) {
			%>
			<tr>
				<td><%=method%></td>
